...
import { z } from 'zod';
import { uuidSchema, coordinatesSchema } from './common';
...
export const createTrafficSnapshotSchema = z.object({
  ...
  affected_area: z.string().max(500).optional()
    .transform(val => val ? sanitizeString(val) : val),
...
export const listTrafficSnapshotsQuerySchema = z.object({
  ...
  condition: z.string().optional()
    .transform(val => val ? sanitizeString(val) : val),
  deliveryStatus: z.string().optional().default('in_transit,delayed')
...
export type ListTrafficSnapshotsQuery = z.infer<typeof listTrafficSnapshotsQuerySchema>;
