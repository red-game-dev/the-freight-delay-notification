...
export function useWorkflowPolling({ workflowId, pollInterval = 3000 }: UseWorkflowPollingOptions) {
  const [workflowJustStarted, setWorkflowJustStarted] = useState(false);
  const [shouldPoll, setShouldPoll] = useState(false);

  // Query workflow status with conditional polling
  const { data: workflowStatus, ...rest } = useWorkflowStatus(workflowId || '', {
  ...
  const shouldEnablePolling = workflowJustStarted || (workflowStatus ? !TERMINAL_STATUSES.includes(workflowStatus.status) : false);
  ...
  const isTerminal = workflowStatus ? TERMINAL_STATUSES.includes(workflowStatus.status) : false;
