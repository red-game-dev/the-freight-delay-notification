...
async function main() {
  ...
  logger.info(`   Twilio: ${process.env.TWILIO_ACCOUNT_SID ? '✅ Configured' : '❌ Not configured'}`);
  logger.info(`   Google Maps: ${process.env.GOOGLE_MAPS_API_KEY ? '✅ Configured' : '❌ Not configured'}`);
  logger.info(`   Mapbox: ${process.env.MAPBOX_ACCESS_TOKEN ? '✅ Configured' : '❌ Not configured'}`);
  logger.info('');
