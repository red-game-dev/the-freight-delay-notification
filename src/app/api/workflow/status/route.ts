...
export const GET = createApiHandler(async (request) => {
  ...
  try {
    ...
    if (description.status.name === "RUNNING") {
      try {
        ...
        const causeMsg =
          hasCause(queryError) && hasMessage(queryError.cause)
            ? (queryError.cause as { message: string }).message
            : "";

        if (errorMsg.includes("Workflow Task in failed state")) {
          logger.warn(
            `⚠️ Cannot query workflow ${workflowId} - task in failed state`,
          );

          // Check if it's a non-determinism error
          if (
            causeMsg.includes("Nondeterminism") ||
            causeMsg.includes("does not match")
          ) {
            logger.warn(`⚠️ Non-determinism detected: ${causeMsg}`);
            error =
              "Workflow code was updated after this workflow started. Please cancel this workflow and start a new one.";
          } else {
            error =
              "Workflow task in failed state - check Temporal UI for details";
          }
    ...
    if (
      ...
      logger.info(
        `⚠️ Workflow ${workflowId} not found in Temporal, checking database...`,
      );
      ...
      if (workflowExecution.success && workflowExecution.value) {
        ...
        return Result.ok({
          ...
          status:
            workflowExecution.value.status === "completed"
              ? "completed"
              : "failed",
      ...
      return Result.fail(
        new NotFoundError(
          `Workflow does not exist in Temporal or database history: ${workflowId}`,
        ),
      );
    }
    ...
    return Result.fail(
      new InfrastructureError(
        `Failed to query workflow: ${getErrorMessage(error)}`,
        { cause: error },
      ),
    );
  }
  ...
  if (!description) {
    return Result.fail(
      new InfrastructureError(
        "Unexpected: description is null after successful query",
      ),
    );
  }
