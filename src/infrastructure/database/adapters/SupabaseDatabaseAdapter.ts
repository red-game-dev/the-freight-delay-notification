...
export class SupabaseDatabaseAdapter implements DatabaseAdapter {
      ...
      if (error) {
        ...
        return failure(
          new InfrastructureError(
            `Failed to get customer: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async updateCustomer(
    ...
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to update customer: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async listCustomers(limit = 10, offset = 0): Promise<Result<Customer[]>> {
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to list customers: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async getRouteById(id: string): Promise<Result<Route | null>> {
    try {
      ...
      if (error) {
        ...
        return failure(
          new InfrastructureError(
            `Failed to get route: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
      ...
      const route: Route = {
        ...data,
        origin_coords: pointToCoordinates(data.origin_coords) || {
          x: 0,
          y: 0,
          lat: 0,
          lng: 0,
        },
        destination_coords: pointToCoordinates(data.destination_coords) || {
          x: 0,
          y: 0,
          lat: 0,
          lng: 0,
  ...
  async createRoute(input: CreateRouteInput): Promise<Result<Route>> {
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to create route: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
    ...
    try {
      ...
      > & {
        ...
        origin_coords: input.origin_coords
          ? this.coordinatesToPoint(input.origin_coords)
          : undefined,
        destination_coords: input.destination_coords
          ? this.coordinatesToPoint(input.destination_coords)
          : undefined,
      };
  ...
  async listRoutes(limit = 10, offset = 0): Promise<Result<Route[]>> {
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to list routes: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
      ...
      const routes = (data || []).map((route) => ({
        ...route,
        origin_coords: pointToCoordinates(route.origin_coords) || {
          x: 0,
          y: 0,
          lat: 0,
          lng: 0,
        },
        destination_coords: pointToCoordinates(route.destination_coords) || {
          x: 0,
          y: 0,
          lat: 0,
          lng: 0,
  ...
  async getDeliveryById(id: string): Promise<Result<Delivery | null>> {
    try {
      ...
      if (error) {
        ...
        return failure(
          new InfrastructureError(
            `Failed to get delivery: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
      ...
      const delivery: Delivery = {
        ...
        current_location: data.current_location
          ? pointToCoordinates(data.current_location)
          : null,
  ...
  async createDelivery(input: CreateDeliveryInput): Promise<Result<Delivery>> {
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to create delivery: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
    ...
    try {
      ...
      > & {
        ...
        current_location: current_location
          ? this.coordinatesToPoint(current_location)
          : undefined,
        metadata:
      ...
      if (!updatedDelivery.success || !updatedDelivery.value) {
        return failure(
          new InfrastructureError("Failed to fetch updated delivery"),
        );
      }
  ...
  async listDeliveries(limit = 10, offset = 0): Promise<Result<Delivery[]>> {
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to list deliveries: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
      ...
      const deliveries = (data || []).map((delivery: DeliveryWithJoins) => ({
        ...
        current_location: delivery.current_location
          ? pointToCoordinates(delivery.current_location)
          : null,
  ...
  async getNotificationById(id: string): Promise<Result<Notification | null>> {
    try {
      ...
      if (error) {
        ...
        return failure(
          new InfrastructureError(
            `Failed to get notification: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async createNotification(
    ...
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to create notification: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async updateNotification(
    ...
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to update notification: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async listNotificationsByDelivery(
    ...
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to list notifications: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async createTrafficSnapshot(
    ...
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to create traffic snapshot: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async listTrafficSnapshots(
    ...
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to list traffic snapshots: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async getWorkflowExecutionById(
    ...
    try {
      ...
      if (error) {
        ...
        return failure(
          new InfrastructureError(
            `Failed to get workflow execution: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async createWorkflowExecution(
    ...
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to create workflow execution: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async updateWorkflowExecution(
    ...
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to update workflow execution: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async listWorkflowExecutions(
    ...
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to list workflow executions: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async getThresholdById(id: string): Promise<Result<Threshold | null>> {
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to get threshold: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async getDefaultThreshold(): Promise<Result<Threshold | null>> {
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to get default threshold: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async createThreshold(
    ...
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to create threshold: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async updateThreshold(
    ...
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to update threshold: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async deleteThreshold(id: string): Promise<Result<void>> {
    try {
      ...
      if (thresholdResult.value.is_default) {
        return failure(
          new InfrastructureError("Cannot delete default threshold"),
        );
      }
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to delete threshold: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async listThresholds(limit = 100, offset = 0): Promise<Result<Threshold[]>> {
    try {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to list thresholds: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async incrementChecksPerformed(deliveryId: string): Promise<Result<number>> {
    try {
      const { data, error } = await this.ensureClient().rpc(
        "increment_checks_performed",
        {
      ...
      if (error) {
        return failure(
          new InfrastructureError(
            `Failed to increment checks_performed: ${getErrorMessage(error)}`,
            { error },
          ),
        );
      }
  ...
  async setAuditContext(
    ...
    try {
      // Set user context
      const { error: userError } = await this.ensureClient().rpc(
        "set_audit_user",
        { user_id: userId },
      );
      ...
      if (userError) {
        return failure(
          new InfrastructureError(
            `Failed to set audit user: ${getErrorMessage(userError)}`,
            { error: userError },
          ),
        );
      }
      ...
      const { error: requestError } = await this.ensureClient().rpc(
      ...
      if (requestError) {
        return failure(
          new InfrastructureError(
            `Failed to set audit request ID: ${getErrorMessage(requestError)}`,
            { error: requestError },
          ),
        );
      }
      ...
      return failure(
        new InfrastructureError(
          `Failed to set audit context: ${getErrorMessage(error)}`,
          { error },
        ),
      );
    }
