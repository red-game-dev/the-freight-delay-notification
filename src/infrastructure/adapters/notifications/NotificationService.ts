...
export class NotificationService {
  ...
  private initializeAdapters(): void {
    ...
    if (env.FORCE_NOTIFICATION_MOCK_ADAPTER) {
      logger.info(
        `🧪 [NotificationService] TESTING MODE: Forcing Mock adapters`,
      );
      this.emailAdapters = [new MockEmailAdapter()];
      this.smsAdapters = [new MockSMSAdapter()];
      logger.info(
        "📬 [NotificationService] Initialized adapters (TESTING MODE):",
      );
      logger.info(
        `   Email adapters: ${this.emailAdapters.map((a) => `${a.providerName}(${a.priority})`).join(", ")}`,
      );
      logger.info(
        `   SMS adapters: ${this.smsAdapters.map((a) => `${a.providerName}(${a.priority})`).join(", ")}`,
      );
      return;
    }
  ...
  async send(
    ...
    const adapters =
      channel === "email" ? this.emailAdapters : this.smsAdapters;

    if (adapters.length === 0) {
      return failure(
        new InfrastructureError(`No ${channel} adapters available`, {
          channel,
        }),
      );
    }
    ...
    for (const adapter of adapters) {
      ...
      if (result.success) {
        logger.info(
          `✅ [NotificationService] Successfully sent via ${adapter.providerName}`,
        );
        return result;
      }
      ...
      const errorMessage = !result.success
        ...
        : "Unknown error";
      logger.info(
        `⚠️ [NotificationService] ${adapter.providerName} failed: ${errorMessage}`,
      );
      errors.push({
    ...
    return failure(
      new InfrastructureError(`All ${channel} notification adapters failed`, {
        channel,
