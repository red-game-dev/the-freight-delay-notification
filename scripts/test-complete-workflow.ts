...
async function testCompleteWorkflow() {
  ...
  try {
    ...
    if (result.steps.trafficCheck) {
      ...
      console.log(`   Delay: ${result.steps.trafficCheck.delayMinutes} minutes`);
      console.log(`   Condition: ${result.steps.trafficCheck.trafficCondition}`);
      console.log(`   Normal Duration: ${result.steps.trafficCheck.normalDurationMinutes} min`);
      console.log(`   With Traffic: ${result.steps.trafficCheck.estimatedDurationMinutes} min`);
    }

    if (result.steps.delayEvaluation) {
      console.log('\n📊 Step 2: Delay Evaluation');
      console.log(`   Exceeds Threshold: ${result.steps.delayEvaluation.exceedsThreshold ? 'YES' : 'NO'}`);
      console.log(`   Delay: ${result.steps.delayEvaluation.delayMinutes} min vs Threshold: ${result.steps.delayEvaluation.thresholdMinutes} min`);
      console.log(`   Severity: ${result.steps.delayEvaluation.severity}`);
      console.log(`   Notification Required: ${result.steps.delayEvaluation.requiresNotification ? 'YES' : 'NO'}`);
    }
    ...
    if (result.steps.messageGeneration) {
      ...
      console.log(`   Fallback Used: ${result.steps.messageGeneration.fallbackUsed ? 'YES' : 'NO'}`);
      if (result.steps.messageGeneration.tokens) {
        console.log(`   Tokens: ${result.steps.messageGeneration.tokens}`);
      }
      console.log(`\n   Message Preview:`);
      console.log(`   ${result.steps.messageGeneration.message.substring(0, 200)}...`);
    }
    ...
    if (result.steps.notificationDelivery) {
      ...
      console.log(`   Sent: ${result.steps.notificationDelivery.sent ? 'YES' : 'NO'}`);
