...
const CRON_INTERVAL_SECONDS = parseInt(process.env.CRON_INTERVAL_SECONDS || '10', 10);
...
console.log(`⏱️  Interval: ${intervalSeconds} seconds (${intervalSeconds / 60} minutes)`);
...
async function runTrafficCheck() {
  ...
  try {
    ...
    errorCount++;
    const duration = Date.now() - startTime;
    console.error(`❌ Run #${runCount} failed after ${duration}ms:`, error.message);
  ...
  // Print statistics
  console.log(`📈 Stats: ${successCount} success, ${errorCount} errors, ${runCount} total`);
}
...
process.on('SIGINT', () => {
  ...
  console.log(`   Successful: ${successCount} (${((successCount / runCount) * 100).toFixed(1)}%)`);
  console.log(`   Failed: ${errorCount} (${((errorCount / runCount) * 100).toFixed(1)}%)`);
  console.log('👋 Goodbye!\n');
  process.exit(0);
});
