...
async function checkCronEndpoint() {
  try {
    const response = await fetch('http://localhost:3000/api/cron/check-traffic', {
      headers: {
...
async function checkTemporal() {
  try {
    const response = await fetch('http://localhost:8233', { signal: AbortSignal.timeout(5000) });
    if (response.ok) {
      results.push({
...
function printResults() {
  ...
  for (const result of results) {
    ...
    if (result.details) {
      const details = JSON.stringify(result.details, null, 2).split('\n').slice(1, -1).join('\n   ');
      console.log(`   ${details}`);
    }
  ...
  if (unhealthyCount === 0) {
    ...
    console.log('⚠️  Some components are not healthy. Please review the errors above.');
    console.log('');

    if (results.find((r) => r.component === 'Next.js Dev Server' && r.status === 'unhealthy')) {
      console.log('💡 Start Next.js: npm run dev');
    }
    if (results.find((r) => r.component.includes('Temporal') && r.status === 'unhealthy')) {
      console.log('💡 Start Temporal: Ensure temporal worker is running (pnpm run temporal:worker)');
    }
    if (results.find((r) => r.component === 'Supabase (Remote)' && r.status === 'unhealthy')) {
      console.log('💡 Check Supabase credentials in .env.local');
    }
    if (results.find((r) => r.component === 'Environment Variables' && r.status === 'unhealthy')) {
...
async function runHealthCheck() {
  ...
  if (results.find((r) => r.component === 'Next.js Dev Server' && r.status === 'healthy')) {
